    ...
    function loadAssetsList(assetsData) {
        ...
        //根据资源配置计算资源总数
        for (var id in assetsData){
            progressTotal++;
        }

        //根据资源配置加载资源
        var i=1;for (var assetId in assetsData)
        {if(typeof(myfirsttext)!='undefined')myfirsttext.innerHTML='正在加载第'+(i++)+'/'+assetsData.length+'个资源';
            assetsAdd(assetsData[assetId], onProgress,assetsData);//console.log(assetsData);
        }
if(typeof(myfirsttext)!='undefined')myfirsttext.innerHTML='资源已加载，正在布置场景！';
    }

    function assetsAdd (asset, callback,assetsData) {//console.log(assetsData);
        // raw json data
        var assetJson = asset;
        let request = null;
