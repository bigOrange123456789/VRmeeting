...
Web3DEngine.ExtendType( SceneSet , Web3DEngine.MonoBehaviour, {
	...
	Start:function(arg){
        ...
        firstChair.transform.localEulerAngles=new THREE.Vector3(0,180,0);
        firstChair.getComponent(Web3DEngine.Transform).localPosition.set(0,-2,0);
        for(var i=0;i<11;i++){
            for(var j=0;j<12;j++){
                var newChair=Web3DEngine.GameObject.Instantiate(firstChair);
                firstChair.getComponent(Web3DEngine.Transform).localPosition.set(1.5-0.26*i,0,2-0.5*j);
