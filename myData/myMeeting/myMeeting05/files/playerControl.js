...
Web3DEngine.ExtendType( PlayerControl , Web3DEngine.MonoBehaviour, {
    ...
    Start:function()
    {
        ...
        var sphere =new THREE.CubeGeometry(100,1,100);// new THREE.SphereGeometry(0.04,60,16);
        var material = new THREE.MeshPhongMaterial({color: 0xffffff});//new THREE.MeshBasicMaterial({color: 0xffffff});
        var square1= new THREE.Mesh(sphere, material);square1.receiveShadow = true;
        square1.position.x=0;square1.position.y=-1.11;square1.position.z=0;
        scene.add(square1);
    ...
    Update:function(arg){
        ...
        if(this.flag1==0.5){
            ...
            for(var i=-4;i<=4;i++)
            if(this.main_user.getComponent(Web3DEngine.Transform).localPosition.x>i-0.03&&this.main_user.getComponent(Web3DEngine.Transform).localPosition.x<i+0.03)this.flag1=0.5;
        ...
        if(this.mytimer<frame_num/8){//0-10
            ...
            move(this.directionY_Ref,2,-step_y*0.8);
            move(this.directionY_Ref,-3,step_z);
        }else{
            ...
            //开始管理个人名片
            for(var i=0;i<sprite_all.length;i++)
                if(Math.abs(sprite_all[i].position.x-this.gameObject.getComponent(Web3DEngine.Transform).localPosition.x)<2&&Math.abs(sprite_all[i].position.z-this.gameObject.getComponent(Web3DEngine.Transform).localPosition.z)<2)appInst._renderScenePass.scene.add(sprite_all[i]);
                else appInst._renderScenePass.scene.remove(sprite_all[i]);
            //结束管理个人名片
        }
...
async  function find_all_user(myman1,all_user,all_user_id){//找到房间中所有用户的位置，并把他们渲染出来
    ...
    var result_avatar=await asyncAxios.get(url + "/myavatar/myfind",{});
    //console.log(result_avatar.result);
    var atThisHouse=new Array();
    for(var i=0;i<all_user.length;i++)atThisHouse.push(0);//atThisHouse的长度比result.result小1
    for(var i=0;i<result.result.length;i++){
        //console.log(result.result[i]);
        var updateOne_user_flag=updateOne_user(myman1,all_user,all_user_id,result.result[i].user_id,result.result[i].x,result.result[i].y,result.result[i].angle1,result.result[i].angle2,result.result[i].angle3,result.result[i].animation,result_avatar);
...
function updateOne_user(myman1, all_user, all_user_id, this_user_id, this_user_x, this_user_y, this_user_angle1, this_user_angle2, this_user_angle3,animation,result_avatar) {
    ...
    //console.log(all_user_id[this_user_id]);
    if (this_user_id != user_id)//不是自己的信息
        if (updateOne_user_flag == -1) {//如果是新来的成员
            for(var j=0;j<result_avatar.result.length;j++)if(result_avatar.result[j].user_id==this_user_id){
                //alert(123);
                var man = new Web3DEngine.GameObject();///模型
                var renderer = man.addComponent(Web3DEngine.SkinnedMeshRenderer);///蒙皮网格渲染器
                renderer.mesh = myman1;

                man.getComponent(Web3DEngine.Transform).localScale = new THREE.Vector3(0.5, ((result_avatar.result[j].bone0 - 0.5) / 1 + 1)/2, 0.5);
                ...
                adjustBone(1,result_avatar.result[j].bone1,renderer,boneArray);
                adjustBone(2,result_avatar.result[j].bone2,renderer,boneArray);
                adjustBone(3,result_avatar.result[j].bone3,renderer,boneArray);
                adjustBone(4,result_avatar.result[j].bone4,renderer,boneArray);
                adjustBone(5,result_avatar.result[j].bone5,renderer,boneArray);
                adjustBone(6,result_avatar.result[j].bone6,renderer,boneArray);
                adjustBone(7,result_avatar.result[j].bone7,renderer,boneArray);
                adjustBone(8,result_avatar.result[j].bone8,renderer,boneArray);
                adjustBone(9,result_avatar.result[j].bone9,renderer,boneArray);
                adjustBone(10,result_avatar.result[j].bone10,renderer,boneArray);
                adjustBone(11,result_avatar.result[j].bone11,renderer,boneArray);
                adjustBone(12,result_avatar.result[j].bone12,renderer,boneArray);
                adjustBone(13,result_avatar.result[j].bone13,renderer,boneArray);
                adjustBone(14,result_avatar.result[j].bone14,renderer,boneArray);
                adjustBone(15,result_avatar.result[j].bone15,renderer,boneArray);
                adjustBone(16,result_avatar.result[j].bone16,renderer,boneArray);

                break;
            }
            all_user_id.push(this_user_id);
            var this_user;if(j<result_avatar.result.length) this_user=man;
            else {
                this_user= creater(myman1, true);
